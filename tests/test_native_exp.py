...
def test_native_exp_registry_placement():
    ...
    # tests/test_native_cross_entropy_core.py); E6 then landed the
    # differentiable NativeTensor.cross_entropy over it
...
def test_native_exp_scope_boundaries_hold():
    ...
    x = NativeTensor.from_array(VALUES)
    core = cpp.NativeTensorCore.from_array(VALUES)
    # (`cross_entropy` is no longer listed here: E6 shipped it as a
    # differentiable NativeTensor operation. It is still absent from the
