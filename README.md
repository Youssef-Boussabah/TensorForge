...
it; the latest completed phase is Phase J.** I0 was the design lock and
...
**Phase K is the latest phase, and only K0 through K8 have landed.** **Phase J is the latest completed phase**, and it remains complete. Phase J was approved *after*
...
There is **no `max`**, no `max_with_indices`, no `argmin`, no general `gather`, no `scatter`, no embedding lookup, no `index_select` backward, no integer arithmetic or reduction, no integer autograd, parameter, buffer, optimizer state, or checkpoint entry, and no casting or promotion, and **K9 is unstarted**.
