...
float32 CPU support — is the latest phase; milestones I0 and I1 are
complete and I2–I11 are not started.** I0 was the design lock and
...
whole phase. **The native runtime is still float64 CPU only**: `float32`
remains listed as unsupported, it is allocatable through the C ABI and
consumed by no operation (every kernel not yet generalized rejects a
float32 handle before touching memory), and the native
