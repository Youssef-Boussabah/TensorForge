...
**Phase K is the latest phase, and only K0 has landed.** **Phase J is the latest completed phase**, and it remains complete. Phase J was approved *after*
...
example, no benchmark, no CTest, and no dependency. Every inventory is
exactly what Phase J left — **54** exported `tf_*` symbols, **25**
experimental names, **24** native CTests, **16** examples, **9**
...
supported native tensor dtype**, there is no native integer storage, no
native `argmax`, and no index selection, and **K1 through K9 are
unstarted** — runtime capability begins at K1. What K0 shipped is
