...
**Phase K is the latest phase, and only K0 and K1 have landed.** **Phase J is the latest completed phase**, and it remains complete. Phase J was approved *after*
...
phase, and K0 and K1 have landed.** **K0 added no
...
Every inventory is
exactly what Phase J left except the native CTest count, which K1 took
from 24 to **25** — **54** exported `tf_*` symbols, **25**
experimental names, **16** examples, **9**
benchmarks — and so is every registry:
`SUPPORTED_DTYPES == ("float64", "float32")`, `SUPPORTED_DEVICES ==
("cpu",)`, `UNSUPPORTED == ("cuda", "amp")`, `RAW_KERNEL_DTYPES ==
("float64",)`, checkpoint version **3** with `(1, 2, 3)` accepted, and
optimizer, loader, and sampler state at version **1**. **`int64` is not a
supported native tensor dtype**, `normalize_dtype("int64")` still raises,
`INDEX_DTYPES` does not exist, there is no public integer constructor, no
native `argmax`, and no index selection, and **K2 through K9 are
unstarted** — public construction begins at K2. What K0 shipped is
