...
float32 CPU support — is the latest phase; it has begun at milestone I0,
which is the design lock and documentation reconciliation only.** I0
ships
[docs/native_dtype_float32_design.md](docs/native_dtype_float32_design.md)
and its guardrail tests and nothing else, so **the native runtime is
still float64 CPU only**: `float32` remains listed as unsupported, the
library still exports exactly 52 `tf_*` symbols, and the native
checkpoint format is still version 2 with versions 1 and 2 accepted. The
contract plans **exactly two** new C ABI symbols for the whole phase
(52 → **54**), rejects per-operation float32 exports, forbids casting,
