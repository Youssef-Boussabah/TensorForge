...
## Phase H — native CPU performance and runtime efficiency, **begun (H0–H4)**
...
registry, no dtype, no device, and no checkpoint format; H1, H2, H3, and
H4 moved no capability either. `UNSUPPORTED` still reads
...
| H5–H8 | **Proposals, not commitments.** Ordered by measured leverage and each explicitly conditional: a milestone whose premise the preceding measurement does not confirm is narrowed, reordered, or dropped, and the reason is recorded in the design | **Not started** |
