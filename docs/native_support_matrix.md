...
registry, no dtype, no device, and no checkpoint format; H1 and H2 moved
no capability either. `UNSUPPORTED` still reads
...
| H3–H8 | **Proposals, not commitments.** Ordered by measured leverage and each explicitly conditional: a milestone whose premise the preceding measurement does not confirm is narrowed, reordered, or dropped, and the reason is recorded in the design | **Not started** |
