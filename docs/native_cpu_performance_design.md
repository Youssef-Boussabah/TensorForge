...
## 19. Candidate designs: adopt, adapt, reject

A CPU-performance phase has a wide menu of standard framework techniques
available to it, and choosing nothing is as much a decision as choosing
something. Each candidate Phase H weighed gets an explicit decision here,
with the reasoning that settled it, so a later reader can tell a
deliberate rejection from an oversight.

| Candidate design | Decision | Reasoning |
|---|---|---|
| A per-device operation directory (`src/ops/cpu/*.cpp`, `src/ops/cuda/*.cpp`, …) | **Adapt** | TensorForge already organizes `cpp/src/` by concern (`elementwise`, `matmul`, `reduction`, `conv2d`, `pooling`, `classification`, `random`, `storage`, `error`) — the same idea without the device axis, which TensorForge does not have. Keep the existing layout; do not add a `cpu/` level for a project with one device. |
| A naive CPU matmul (triple loop, no blocking/SIMD/OpenMP/BLAS) as the only shipped path | **Adopt as a reference, reject as the shipped path** | Leaving a CPU matmul naive is defensible when a GPU path carries the performance story. TensorForge has no GPU path to fall back on, and §3.1/B2 measured 3.3× available with bit-identical results. Keep a naive kernel as the retained reference (§8.3); do not keep it as the only path. **Done at H2**: `tf::matmul_generic_strided` is the naive kernel, retained verbatim, shipped, and reachable through ordinary dispatch; `tf::matmul_row_sweep` is the second path beside it. |
| A default-OFF `WITH_AVX2` CMake option (`/arch:AVX2` on MSVC, `-mavx2` elsewhere) | **Reject for now; adopt the *shape* if §11 is ever met** | The default-off, single-option, no-intrinsics-in-source form is the right shape, and is what TensorForge would use. But adopting it now would be optimizing the second-order term before the first (§11), and TensorForge's evidence said the win was in loop order — which **H2 then confirmed by measurement**, recovering 4.1–4.7× at the profile shape with no intrinsics and no build-system change. Revisit only under §11. |
| A general fused-operation layer | **Reject for Phase H** | Fusion is a semantic change: it creates new operations with new backward contracts, new saved state, and new versioning obligations. TensorForge's fused kernels (softmax, log-softmax, cross-entropy, dropout) each got their own milestone with its own contract for exactly that reason. Fusion is a future *capability* phase, not a performance milestone. |
| A memory pool with block splitting, coalescing, and stream-aware behavior | **Reject for now; criteria recorded** | Pooling's usual motivation is GPU allocation latency, which TensorForge does not have. §3.1/B1 shows TensorForge's allocation cost is dominated by the *eager zero-fill*, not by the allocator — a much cheaper fix with no ownership risk. §10 records what would have to be true first. |
| Scratch workspaces | **Reject for now; criteria recorded** | TensorForge's evidence does not identify a kernel whose cost a persistent scratch buffer would remove, and a workspace adds lifetime and reentrancy obligations that need a measured reason to exist. §10 records the criteria. |
| Benchmark flags that isolate one large operation with increased repetitions | **Adopt** | Shipped as `--profile CASE`, with TensorForge's own §5 shape-selection rules and its own correctness-before-timing rule layered on. This is the highest-value profiling affordance in the phase. |
...
| Committed benchmark results in a tracked results file, with epoch-time comparisons | **Reject** | TensorForge deliberately commits **no** machine-specific benchmark result file, in any phase. Every prior characterization benchmark (D12, E9, F7, G8) writes no result file and asserts no speed, and H0 keeps that. A committed number becomes a promise the project cannot keep across machines. |
| Environment-variable kernel selectors (a variable naming which conv2d gradient kernel runs, …) | **Reject** | An environment variable that changes which kernel runs makes results depend on ambient process state, which contradicts §7.4 determinism and would silently invalidate the exact-resume proofs. TensorForge dispatch is metadata-driven and explicit (§8.1). |
| pybind11 bindings behind a `BUILD_PYTHON_BINDINGS` option | **Reject** | TensorForge's plain C-ABI + ctypes boundary is a deliberate architectural choice: no build-time Python dependency, no ABI coupling to a Python version, and a narrow exported surface. Already a standing project decision; Phase H does not revisit it. |
...
| A CUDA architecture (handwritten kernels, cuBLAS dispatch, NCCL, AMP) | **Reject for Phase H; noted as a future dependency** | This is the strongest argument in §1.2, read in reverse: a CPU path can afford to stay naive only when a GPU path carries the performance story. TensorForge has no GPU path, so its CPU runtime has to carry it — and a future CUDA phase will inherit whatever runtime Phase H leaves behind. |
